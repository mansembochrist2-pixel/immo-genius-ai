...
import {
  ...
  ChevronRight, Loader2, Sparkles, Pencil, Trash2, RefreshCw,
...
const Clients = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("prospects").insert({
        ...
        canal_prefere: form.canal_prefere || null,
  ...
  const enrichClient = useCallback(async (client: any) => {
    ...
    try {
      ...
      await supabase.from("prospects").update({
        ...
        score_urgence: data.score_urgence,
        motivation: data.motivation,
        freins: data.freins,
        resume_ia: data.resume_ia,
        strategie_adaptee: data.strategie_adaptee,
  ...
  const startEdit = () => {
    ...
    setForm({
      ...
      statut: selected.statut || "nouveau",
      budget_min: selected.budget_min?.toString() || "",
      budget_max: selected.budget_max?.toString() || "",
      secteur_recherche: selected.secteur_recherche || "",
      type_bien_recherche: selected.type_bien_recherche || "",
      motivation: selected.motivation || "",
      freins: selected.freins || "",
      notes: selected.notes || "",
      canal_prefere: selected.canal_prefere || "",
      ...
      <div className="page-header">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="page-subtitle">Fiches intelligentes auto-évolutives</p>
          </div>
              ...
              filtered.map((client: any) => (
                <Card
                  ...
                  className={`cursor-pointer transition-all hover:border-primary/40 ${
                    selectedId === client.id ? "border-primary/60 bg-primary/5" : "bg-card/60 border-border/30"
                  }`}
                  onClick={() => { setSelectedId(client.id); setEditMode(false); }}
                >
                  <CardContent className="p-3">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      {client.score_ia != null && (
                        <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                          <Brain className="h-3 w-3 text-primary" /> {client.score_ia}/100
                        </span>
                      )}
                      {client.taux_signature != null && Number(client.taux_signature) > 0 && (
                        <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                          <TrendingUp className="h-3 w-3" /> {client.taux_signature}%
                        </span>
                      )}
                      {(client.score_urgence ?? 0) >= 7 && (
          ...
          {selected ? (
            <div className="space-y-4 overflow-y-auto flex-1 min-h-0 pr-1">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                        ...
                        {selected.telephone && (
                          <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {selected.telephone}</span>
                        )}
                        {selected.derniere_interaction && (
                          <span className="flex items-center gap-1">
                            <Clock className="h-3 w-3" /> Dernière interaction: {timeAgo(selected.derniere_interaction)}
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => enrichClient(selected)}
                      >
                        {enrichingId === selected.id ? (
                          <><Loader2 className="h-3 w-3 mr-1 animate-spin" /> Analyse...</>
                        ) : (
                          <><Sparkles className="h-3 w-3 mr-1" /> Recalcul IA</>
                        )}
                      </Button>
                      <Button
                        size="sm" variant="ghost" className="h-8 text-xs text-destructive"
                        onClick={() => { if (confirm("Supprimer ce client ?")) deleteMutation.mutate(selected.id); }}
                      >
                        <Trash2 className="h-3 w-3" />
                      </Button>
                      <Badge className={`${statutColors[selected.statut] || ""}`}>
              ...
              {editMode && (
                <Card className="bg-card/60 border-border/30">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Modifier la fiche</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div><Label className="text-xs">Canal préféré</Label><Input value={form.canal_prefere} onChange={(e) => setForm({ ...form, canal_prefere: e.target.value })} className="bg-muted/10 mt-1" /></div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  <CardContent className="p-3">
                    ...
                    <p className="text-sm font-bold mt-1">
                      {selected.budget_min || selected.budget_max
                        ? `${selected.budget_min ? Number(selected.budget_min).toLocaleString("fr-FR") : "?"}€ - ${selected.budget_max ? Number(selected.budget_max).toLocaleString("fr-FR") : "?"}€`
                        : "—"}
                    </p>
                  </CardContent>
                </Card>
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-3">
                    ...
                    <p className="text-sm font-bold mt-1">{selected.score_ia ?? "—"}<span className="text-muted-foreground font-normal">/100</span></p>
                  </CardContent>
                </Card>
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-3">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Taux signature</p>
                    <p className="text-sm font-bold mt-1">{selected.taux_signature != null ? `${selected.taux_signature}%` : "—"}</p>
                  </CardContent>
                </Card>
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-3">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Urgence relance</p>
                    <p className={`text-sm font-bold mt-1 ${(selected.score_urgence ?? 0) >= 7 ? "text-warning" : ""}`}>
                      {selected.score_urgence ?? "—"}<span className="text-muted-foreground font-normal">/10</span>
                    </p>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-4">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">Motivation</p>
                    <p className="text-sm">{selected.motivation || <span className="italic text-muted-foreground">Lancez le recalcul IA</span>}</p>
                  </CardContent>
                </Card>
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-4">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">Freins</p>
                    <p className="text-sm">{selected.freins || <span className="italic text-muted-foreground">Lancez le recalcul IA</span>}</p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="bg-card/60 border-border/30">
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    {selected.resume_ia || "Cliquez sur « Recalcul IA » pour générer un résumé stratégique."}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="bg-card/60 border-border/30">
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    {selected.strategie_adaptee || "Cliquez sur « Recalcul IA » pour des recommandations personnalisées."}
                  </p>
                </CardContent>
              </Card>
                ...
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2">
                    ...
                    {interactions.length > 0 && (
                      <Badge variant="secondary" className="text-[9px]">{interactions.length}</Badge>
                    )}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {interactions.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">
                      Aucune interaction rattachée. Liez des messages inbox à ce client pour alimenter l'historique.
                    </p>
                  ) : (
                          ...
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="outline" className="text-[8px] px-1">
                                {msg.direction === "entrant" ? "Reçu" : "Envoyé"}
                              </Badge>
                            </div>
              ...
              {/* Tags & metadata */}
              {(selected.tags?.length > 0 || selected.secteur_recherche || selected.type_bien_recherche) && (
                <Card className="bg-card/60 border-border/30">
                  <CardContent className="p-4">
                    <div className="flex flex-wrap gap-2">
                      {selected.secteur_recherche && (
                        <Badge variant="outline" className="text-[10px]">📍 {selected.secteur_recherche}</Badge>
                      )}
                      {selected.type_bien_recherche && (
                        <Badge variant="outline" className="text-[10px]">🏠 {selected.type_bien_recherche}</Badge>
                      )}
            ...
            <Card className="bg-card/60 border-border/30 flex-1 flex items-center justify-center min-h-[400px]">
              <div className="text-center space-y-3">
                ...
                <p className="text-xs text-muted-foreground/60">
                  Cliquez sur un client à gauche pour voir sa fiche intelligente
                </p>
              </div>
            </Card>
          )}
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label className="text-xs">Email</Label><Input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="mt-1" placeholder="jean@email.com" /></div>
            <div><Label className="text-xs">Téléphone</Label><Input value={form.telephone} onChange={(e) => setForm({ ...form, telephone: e.target.value })} className="mt-1" placeholder="06 12 34 56 78" /></div>
            <div>
              <Label className="text-xs">Statut</Label>
              <Select value={form.statut} onValueChange={(v) => setForm({ ...form, statut: v })}>
            ...
            <div><Label className="text-xs">Secteur recherché</Label><Input value={form.secteur_recherche} onChange={(e) => setForm({ ...form, secteur_recherche: e.target.value })} className="mt-1" placeholder="Paris 11ème" /></div>
            <div><Label className="text-xs">Type de bien</Label><Input value={form.type_bien_recherche} onChange={(e) => setForm({ ...form, type_bien_recherche: e.target.value })} className="mt-1" placeholder="T3, maison..." /></div>
            <div className="col-span-2"><Label className="text-xs">Notes</Label><Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="mt-1 min-h-[60px]" placeholder="Informations complémentaires..." /></div>
          </div>
          <DialogFooter>
