...
import {
  ...
  MapPin, Home, DollarSign, Timer, User, Download,
} from "lucide-react";
...
export const ClientDetail = ({ client, interactions, enrichingId, onEnrich, onDeleted }: ClientDetailProps) => {
  ...
  const { data: profile } = useQuery({
    queryKey: ["profile"],
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("full_name, agency_name").eq("id", user!.id).single();
      return data;
    },
    enabled: !!user,
  });
  ...
  const saveEdit = () => {
    updateMutation.mutate({
      nom: form.nom, email: form.email || null, telephone: form.telephone || null, statut: form.statut,
      budget_min: form.budget_min ? Number(form.budget_min) : null, budget_max: form.budget_max ? Number(form.budget_max) : null,
      secteur_recherche: form.secteur_recherche || null, type_bien_recherche: form.type_bien_recherche || null,
      type_projet: form.type_projet || null, delai_projet: form.delai_projet || null,
      situation: form.situation || null, motivation: form.motivation || null, freins: form.freins || null,
  ...
  return (
    <div className="space-y-4 overflow-y-auto flex-1 min-h-0 pr-1">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button size="sm" variant="ghost" className="h-7 text-xs text-destructive" onClick={() => { if (confirm(lang === "fr" ? "Supprimer ?" : "Delete?")) deleteMutation.mutate(); }}><Trash2 className="h-3 w-3" /></Button>
      ...
      {/* IA Recommendation */}
      <Card className="bg-card/60 border-primary/20 glow-border card-shimmer">
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Zap className="h-4 w-4 text-primary" /> {lang === "fr" ? "Recommandation IA" : "AI Recommendation"}</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div className="bg-muted/10 rounded-lg p-3 text-center">
              <p className="text-[10px] text-muted-foreground uppercase">{lang === "fr" ? "Prochaine action" : "Next action"}</p>
              <p className="text-xs font-medium mt-1">{client.strategie_adaptee ? client.strategie_adaptee.split(".")[0] + "." : (lang === "fr" ? "Lancez le recalcul IA" : "Run AI analysis")}</p>
            </div>
            <div className="bg-muted/10 rounded-lg p-3 text-center">
      ...
      {editMode && (
        <Card className="bg-card/60 border-border/30">
          <CardHeader className="pb-2"><CardTitle className="text-sm">{lang === "fr" ? "Modifier la fiche" : "Edit client"}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div>
              <p className="text-[10px] uppercase text-muted-foreground font-semibold tracking-wider mb-2">{lang === "fr" ? "Identité" : "Identity"}</p>
              <div className="grid grid-cols-2 gap-3">
                <div><Label className="text-xs">{lang === "fr" ? "Nom *" : "Name *"}</Label><Input value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })} className="bg-muted/10 mt-1" /></div>
                ...
                <div><Label className="text-xs">{lang === "fr" ? "Téléphone" : "Phone"}</Label><Input value={form.telephone} onChange={e => setForm({ ...form, telephone: e.target.value })} className="bg-muted/10 mt-1" /></div>
                <div><Label className="text-xs">{lang === "fr" ? "Statut" : "Status"}</Label>
                ...
                <div><Label className="text-xs">Source</Label><Input value={form.source} onChange={e => setForm({ ...form, source: e.target.value })} className="bg-muted/10 mt-1" /></div>
                <div><Label className="text-xs">{lang === "fr" ? "Canal préféré" : "Preferred channel"}</Label><Input value={form.canal_prefere} onChange={e => setForm({ ...form, canal_prefere: e.target.value })} className="bg-muted/10 mt-1" /></div>
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase text-muted-foreground font-semibold tracking-wider mb-2">{lang === "fr" ? "Informations projet" : "Project info"}</p>
              <div className="grid grid-cols-2 gap-3">
                <div><Label className="text-xs">{lang === "fr" ? "Type de projet" : "Project type"}</Label>
                  <Select value={form.type_projet} onValueChange={v => setForm({ ...form, type_projet: v })}>
                    <SelectTrigger className="bg-muted/10 mt-1"><SelectValue placeholder="—" /></SelectTrigger>
                ...
                <div><Label className="text-xs">Situation</Label>
            ...
            <div>
              <p className="text-[10px] uppercase text-muted-foreground font-semibold tracking-wider mb-2">{lang === "fr" ? "Suivi commercial" : "Sales tracking"}</p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2"><Label className="text-xs">{lang === "fr" ? "Biens proposés" : "Properties offered"}</Label><Textarea value={form.biens_proposes} onChange={e => setForm({ ...form, biens_proposes: e.target.value })} className="bg-muted/10 mt-1 min-h-[50px]" /></div>
                <div><Label className="text-xs">{lang === "fr" ? "Prochain rappel" : "Next reminder"}</Label><Input type="datetime-local" value={form.prochain_rappel} onChange={e => setForm({ ...form, prochain_rappel: e.target.value })} className="bg-muted/10 mt-1" /></div>
                <div><Label className="text-xs">{lang === "fr" ? "Note du rappel" : "Reminder note"}</Label><Input value={form.prochain_rappel_note} onChange={e => setForm({ ...form, prochain_rappel_note: e.target.value })} className="bg-muted/10 mt-1" /></div>
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase text-muted-foreground font-semibold tracking-wider mb-2">{lang === "fr" ? "Analyse qualitative" : "Qualitative analysis"}</p>
              <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button size="sm" onClick={saveEdit} disabled={!form.nom?.trim() || updateMutation.isPending}>
                {updateMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("common.save")}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {!editMode && (client.type_projet || client.delai_projet || client.situation || client.biens_proposes || client.prochain_rappel) && (
        <Card className="bg-card/60 border-border/30">
          <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Home className="h-4 w-4 text-primary" /> {lang === "fr" ? "Projet & Suivi" : "Project & Follow-up"}</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
              ...
              {client.type_bien_recherche && <div><p className="text-[10px] text-muted-foreground uppercase">{lang === "fr" ? "Bien recherché" : "Property sought"}</p><p className="font-medium mt-0.5">{client.type_bien_recherche}</p></div>}
              {client.delai_projet && <div><p className="text-[10px] text-muted-foreground uppercase">{lang === "fr" ? "Délai" : "Timeline"}</p><p className="font-medium mt-0.5">{client.delai_projet}</p></div>}
            ...
            {client.prochain_rappel && (
              <div className="mt-3 bg-warning/10 rounded-lg p-2 flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-medium">{lang === "fr" ? "Rappel" : "Reminder"} : {new Date(client.prochain_rappel).toLocaleDateString("fr-FR", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
      ...
      {/* AI Summary */}
      <Card className="bg-card/60 border-border/30">
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Brain className="h-4 w-4 text-primary" /> {t("clients.ai_summary")}</CardTitle></CardHeader>
        <CardContent><p className="text-sm text-muted-foreground">{client.resume_ia || (lang === "fr" ? "Cliquez sur « Recalcul IA » pour générer un résumé stratégique." : "Click 'AI Recalculation' to generate a strategic summary.")}</p></CardContent>
      </Card>

      {/* Strategy */}
      <Card className="bg-card/60 border-border/30">
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Target className="h-4 w-4 text-primary" /> {t("clients.strategy")}</CardTitle></CardHeader>
        <CardContent><p className="text-sm text-muted-foreground">{client.strategie_adaptee || (lang === "fr" ? "Cliquez sur « Recalcul IA » pour des recommandations." : "Click 'AI Recalculation' for recommendations.")}</p></CardContent>
      </Card>
      ...
      <Card className="bg-card/60 border-border/30">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <MessageSquare className="h-4 w-4 text-primary" /> {t("clients.interactions")}
            {interactions.length > 0 && <Badge variant="secondary" className="text-[9px]">{interactions.length}</Badge>}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {interactions.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">{t("clients.no_interactions")}</p>
          ) : (
                ...
                <div key={msg.id} className="bg-muted/10 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-[8px] px-1">{msg.direction === "entrant" ? (lang === "fr" ? "Reçu" : "Received") : (lang === "fr" ? "Envoyé" : "Sent")}</Badge>
                    </div>
