...
export function NotificationBell() {
  ...
  const { data: alerts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Nouvelles annonces de pige à fort potentiel (24h)
      const last24h = new Date(Date.now() - 86400000).toISOString();
      const { data: hotPige } = await supabase
        .from("annonces_pige")
        .select("id, titre, score_pigeabilite")
        .gte("created_at", last24h)
        .gte("score_pigeabilite", 70)
        .limit(3);
      (hotPige || []).forEach((p: any) => {
        results.push({ id: `pige-${p.id}`, text: `Pige chaude : ${p.titre} (${p.score_pigeabilite}/100)`, type: "info", route: "/chasseur", icon: Crosshair });
      });

      // Nouvelles opportunités Radar
      const { data: newOpps } = await supabase
        .from("opportunites").select("id, titre").eq("statut", "nouvelle").limit(3);
      (newOpps || []).forEach((o: any) => {
        results.push({ id: `opp-${o.id}`, text: `Opportunité : ${o.titre}`, type: "info", route: "/chasseur", icon: Zap });
      });

      // Actions critiques
      const { data: critActions } = await supabase
        .from("actions_recommandees").select("id, titre").eq("statut", "en_attente").eq("priorite", "critique").limit(3);
      (critActions || []).forEach((a: any) => {
  ...
  const dismissAlert = (e: React.MouseEvent, id: string) => { e.stopPropagation(); setDismissed(prev => new Set(prev).add(id)); };
          ...
          <Card className="absolute right-0 top-full mt-2 w-[340px] z-50 bg-card border-border/50 shadow-2xl">
            <CardHeader className="pb-2 pt-3 px-4 flex flex-row items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2">
                <Bell className="h-3.5 w-3.5 text-primary" /> Alertes
              ...
              {visibleAlerts.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {visibleAlerts.map(a => {
                    const Icon = a.icon;
                    return (
                      <div key={a.id} className={cn("p-2.5 rounded-lg cursor-pointer text-xs transition-colors flex items-start gap-2 group", typeStyles[a.type])} onClick={() => { navigate(a.route); setOpen(false); }}>
                        <Icon className={cn("h-3.5 w-3.5 shrink-0 mt-0.5", a.type === "destructive" ? "text-destructive" : a.type === "warning" ? "text-warning" : "text-info")} />
                        <span className="flex-1 leading-relaxed">{a.text}</span>
                        <button className="opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5" onClick={(e) => dismissAlert(e, a.id)}>
